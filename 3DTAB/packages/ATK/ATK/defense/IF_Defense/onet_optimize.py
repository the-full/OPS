...
class ONetOpt(BasicDefense):
    ...
    def optimize_points(
        ...
        for i in range(iterations + 1):
            # 1. occ = threshold
            occ_value = self.generator.model.decode(opt_points, z, c).logits
    ...
    def defense(self, adv_pcs, labels, **kwargs):
        ...
        opt_pc = np.zeros((len(pc), self.sample_npoint, 3), dtype=np.float32)
            ...
            for idx in pbar:
                ...
                with torch.no_grad():
                    ...
                    z = self.generator.model.get_z_from_prior(
                        (len(batch_pc),), sample=self.generator.sample).cuda()
