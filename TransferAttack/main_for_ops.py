...
def get_parser():
    ...
    parser.add_argument('--num_sample_neighbor', default=20, type=int)
    parser.add_argument('--num_sample_operator', default=20, type=int)
    parser.add_argument('--level_list_range_end', default=5, type=int)
    parser.add_argument('--radius_list_range_start', default=0.25, type=float)
    parser.add_argument('--radius_list_range_gap', default=0.25, type=float)
    parser.add_argument('--radius_list_range_end', default=1.5, type=float)
    return parser.parse_args()


def main():
    # NOTE: Initialize a tracker to monitor GPU memory usage.
    ...
    if not args.eval:
        ...
        if args.attack == 'ops' or args.attack == 'ops_only_ns':
            r_start = args.radius_list_range_start
            r_gap = args.radius_list_range_gap
            r_end = args.radius_list_range_end
            attacker = transferattack.load_attack_class(args.attack)(
                ...
                sample_levels       = range(2, args.level_list_range_end),
                sample_ratios       = np.arange(r_start, r_end + r_gap, r_gap),
            )
        else:
        ...
        # NOTE: Retrieve the maximum GPU memory allocated during code execution and output the result.
        max_mem_allocated = torch.cuda.max_memory_allocated()
        print(f"Maximum memory allocated during the code execution: {max_mem_allocated / (1024 ** 2):.2f} MB")
    else:
        asr = dict()
        res = '|'
