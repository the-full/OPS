...
[英文](https://github.com/the-full/OPS/blob/main/TransferAttack/README.md)

## 1. 简介

本文件夹中的代码基于公开基准 **TransferAttack**，用于评估图像分类任务中的对抗样本可迁移性。
原始 `README.md` 的内容保留在文档结尾供参考。

## 2. 环境配置

请按照以下步骤配置 `TransferAttack` 环境：
...
3. **（可选）使用预生成的对抗样本**
   ...
   * 解压后，将内容放置到以下目录：
     ...
     | **源路径（exp_data-TransferAttack）** | **目标路径**                   |
     | --------------------------------- | -------------------------- |
     | `exp_data/`                       | `TransferAttack/exp_data/` |

   * 如果你使用了这些预生成的样本，请**务必跳过**实验脚本中的对抗样本生成命令，避免覆盖已有文件。

## 3. 运行实验

环境配置完成后，可以开始运行实验。请按照以下步骤操作：

1. 激活 `TransferAttack` 环境：

   ```bash
   conda activate TransferAttack
   ```

2. 执行某个 `run_exp_*.sh` 脚本以启动实验，例如：

   ```bash
   bash run_exp_OPS_diff_overhead.sh
   ```

   **注意**：如果你使用了第 2.3 步的预生成对抗样本，请**务必跳过**实验脚本中的样本生成部分，以防止覆盖已有数据，并确保实验使用预生成数据。

   每个脚本将执行特定的评估任务，结果将保存到 `result.txt` 文件中（部分防御方法除外）。实验脚本与论文中实验的对应关系如下：

   | **脚本名称**                              | **对应论文图表**    |
   | ------------------------------------- | ------------- |
   | `run_exp_baseline_method.sh`          | 表 1 和 图 2     |
   | `run_exp_OPS_diff_overhead.sh`        | 表 1 和 图 2、图 3 |
   | `run_exp_OPS_diff_overhead_extra.sh`  | 图 2 和 图 3     |
   | `run_exp_OPS_diff_overhead_extra2.sh` | 未使用           |
   | `run_exp_only_neighbor_sample.sh`     | 未使用           |
   | `run_exp_only_operator_sample.sh`     | 未使用           |
   | `defense/run_exp_defense_at.sh`       | 表 2（Baseline） |
   | `defense/run_exp_defense_at_ops.sh`   | 表 2（OPS）      |
   | `defense/run_exp_defense_hgd.sh`      | 表 2（Baseline） |
   | `defense/run_exp_defense_hgd_ops.sh`  | 表 2（OPS）      |
   | `defense/run_exp_defense_nrp.sh`      | 表 2（Baseline） |
   | `defense/run_exp_defense_nrp_ops.sh`  | 表 2（OPS）      |
   | `defense/run_exp_defense_rs.sh`       | 表 2（Baseline） |
   | `defense/run_exp_defense_rs_ops.sh`   | 表 2（OPS）      |
